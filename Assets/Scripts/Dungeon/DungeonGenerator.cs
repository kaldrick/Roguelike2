...
//	private GameObject floor;
	...
	void Update () {
		...
		if(bOnce)
		{
			...
			Camera.main.transform.position = new Vector3(MAP_WIDTH/2,128,MAP_HEIGHT/2);
...
//		floor.renderer.material.mainTexture = dungeonTexture;
		...
		{
			roomXY.X.Add (room2.boundary.center.x);	
			roomXY.Y.Add (room2.boundary.center.y);	
		}
		roomXY.i = rooms.Count;
		// Place Player
		...
		for(int i = 0; i<16 ; i++)
		{
			...
			int r2 = Random.Range (0, rooms.Count-1);
			Room room2 = rooms[r2];
			GameObject AI = GameObject.Instantiate (prefabAI,new Vector3(room2.boundary.center.x,1.0f,room2.boundary.center.y), transform.rotation) as GameObject;
			AI.name += i;
			AI.transform.parent = GameObject.Find ("AIs").transform;
		}
			...
			for (int row = _quadtree.boundary.BottomTile(); row <= _quadtree.boundary.TopTile()-1; row++)
			{
				for (int col = _quadtree.boundary.LeftTile(); col <= _quadtree.boundary.RightTile()-1; col++)
				{
					...
					else if (id == Tile.TILE_WALL)
					{
						...
						if(Random.Range (0, 128) < 2)
						{
							GameObject light = GameObject.Instantiate (prefabLight, wall.transform.position + new Vector3(0, 1, 0), wall.transform.rotation) as GameObject;	
							light.transform.parent = GameObject.Find("Lights").transform;	
						}
					}
				}
			}
