...
public static function HasKey(key : String){
	var types : String[] = ["bool:{0}","Colour:{0}-r","Colour32:{0}-r","Vector2:{0}-x","Vector3:{0}-x","Vector4:{0}-x","Vector3:Quaternion:{0}-x","Vector4:Rect:{0}-x"];
	var flag : boolean = false;
	for( var type : String in types ){
		if( PlayerPrefs.HasKey("PlayerPrefsPlus:"+String.Format(type,key)) )
			flag = true;
	}
	return flag;
}
