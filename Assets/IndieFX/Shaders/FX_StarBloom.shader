Shader "IndieFX/Shaders/FX StarBloom" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" { }
		_BlurTex ("Base (RGB)", 2D) = "white" { }
		_OffsetScale("Offset", Range (0.0, 10.0)) = 1.0
		_ThresholdMin ("ThresholdMin", Range (0.0, 1.0)) = 0.9
		_ThresholdMax ("ThresholdMax", Range (0.0, 1.0)) = 1.0
	...
	half4 fragOff (v2f_off i) : COLOR
	{
		...
		float4 col = tex2D(_BlurTex, (i.uv[0] + i.uv[1]) * 0.5);
		float4 newCol;
		newCol.rgb = (0, 0, 0);
		int count = 1;
