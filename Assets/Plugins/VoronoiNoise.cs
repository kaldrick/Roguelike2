...
public class VoronoiNoise : ThreadedJob
{
	...
	private   COMBINE_FUNC CombineFunc = CombineFunc_D0;
	...
	public float FractalNoise2D(float x, float y, int octNum, float frq, float amp)
	{
		...
		{
			sum += Noise2D(new Vector2(x*gain/frq, y*gain/frq)) * amp/gain;
			gain *= 2.0f;
		}
		return sum;
	}
...
/*	//Sample 3D fractal noise
	public   float FractalNoise3D(float x, float y, float z, int octNum, float frq, float amp)
	{
		...
		for(int i = 0; i < octNum; i++)
		{
			sum += Noise3D(new Vector3(x*gain/frq, y*gain/frq, z*gain/frq)) * amp/gain;
			gain *= 2.0f;
		}
		return sum;
	}*/
...
/*	//Sample single octave of 3D noise
	...
	*/
	...
	private   void insert(float[] arr, float value)
	{
		float temp;
	...
	private   uint lcgRandom(uint lastValue)
	{
